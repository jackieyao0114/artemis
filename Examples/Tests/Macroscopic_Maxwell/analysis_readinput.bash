...
for alpha in $(seq 0.0001 0.0001 0.5); do
    ...
    ./main3d.gnu.TPROF.MTMPI.CUDA.LLG.GPUCLOCK.ex inputs_3d_LLG_ML > output_$alpha
    ...
    for timestep in $(seq 0 10 5000 ); do
        ...
        bash analysis_getline.bash raw_data${alpha}/${newnumber}.txt 7 | tee -a Mz_zface_${alpha}.txt
        bash analysis_getline.bash raw_data${alpha}/${newnumber}.txt 6 | tee -a My_yface_${alpha}.txt
        bash analysis_getline.bash raw_data${alpha}/${newnumber}.txt 5 | tee -a Mx_xface_${alpha}.txt
    done
    rm -r ./diags
done